...
from src.models.CDEMP.model import CDEMP
...
def train(model, train_set, dev_set):
    ...
    try:
        ...
        for n_iter in tqdm(range(1000000)):
            if "cem" or "cdemp"  or 'cemp' in config.model:
...
def test(model, test_set):
    ...
    file_summary = config.save_path + '/' + str(config.model) +"_results.txt"
    with open(file_summary, "w", encoding='utf-8') as f:
        f.write("EVAL\tLoss\tPPL\tAccuracy\n")
        f.write(
            "{}\t{:.4f}\t{:.4f}\t{:.4f}\n".format(loss_test, ppl_test, bce_test, acc_test)
        )
        for r in results:
            f.write(r)
