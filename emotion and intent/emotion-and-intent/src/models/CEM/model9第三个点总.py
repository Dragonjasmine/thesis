...
from src.models.CEM.other_layer import Sampling
...
class CEM(nn.Module):
    def __init__(self, vocab, decoder_number, model_file_path=None, is_eval=False, load_optim=False):
        ...
        self.res_encoder = self.make_encoder((config.emb_dim))

        self.int_lin = nn.Linear(config.hidden_dim, 9, bias=False)
        self.vae_sampler = Sampling(config.hidden_dim, config.hidden_dim, out_dim=300)
        ...
        self.cog_lin = MLP()
        self.dense = nn.Linear(2 * config.hidden_dim, config.hidden_dim)
    ...
    def save_model(self, running_avg_ppl, iter):
        ...
        model_save_path = os.path.join(
            self.model_dir,
            "CDEMP_{}_{:.4f}".format(iter, running_avg_ppl),
        )
    ...
    def forward(self, batch):
        ...
        if not config.woEMO:
            ...
            emo_ref_ctx, _ = self.attention(emo_ref_ctx, emo_ref_ctx, emo_ref_ctx, None)
            emo_logits = self.emo_lin(emo_ref_ctx[:, 0])
        else:
            emo_ref_ctx = enc_outputs
        ...
        return src_mask, emo_ref_ctx, cog_ref_ctx, emo_logits
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        src_mask, emo_ref_ctx, ctx_output, emo_logits = self.forward(batch)

        mask_src_res = dec_batch.data.eq(config.PAD_idx).unsqueeze(1)
        res_encoder_outputs = self.res_encoder(self.embedding(dec_batch), mask_src_res)

        mu_prior_state, logvar_prior_state, z_prior_state, _ = self.vae_sampler(ctx_output[:, 0])

        if train:
            mu_posterior_state, logvar_posterior_state, z_posterior_state, _ = self.vae_sampler. \
                forward_train(res_encoder_outputs[:, 0])
            kll_loss = self.vae_sampler.kl_div(mu_posterior_state,
                                               logvar_posterior_state,
                                               mu_prior_state,
                                               logvar_prior_state
                                               )
            intent_logits = self.int_lin(z_posterior_state)
            intent_label = torch.LongTensor(batch['program_label_intent']).to(config.device)
            intent_loss = nn.CrossEntropyLoss()(intent_logits, intent_label).to(config.device)

        else:
            kll_loss = self.vae_sampler.kl_div(mu_prior_state, logvar_prior_state)
            intent_logits = self.int_lin(z_prior_state)
            intent_label = torch.LongTensor(batch['program_label_intent']).to(config.device)
            intent_loss = nn.CrossEntropyLoss()(intent_logits, intent_label).to(config.device)
        ##############    z_prior_state = 意图信息
        ...
        #内容-情感
        pre_logit_emo, attn_dist = self.decoder(dec_emb, emo_ref_ctx, ctx_output, (src_mask, mask_trg))
        #内容意图
        pre_logit, attn_dist = self.decoder_1(dec_emb, ctx_output, (src_mask, mask_trg))
        pre_logit_int, attn_dist = self.decoder_2(pre_logit, z_prior_state.unsqueeze(1), (None, mask_trg))
        #最终生成
        pre_logit = torch.cat([pre_logit_int, pre_logit_emo], dim=-1)
        cog_contrib = nn.Sigmoid()(pre_logit)
        cog_ref_ctx = cog_contrib * pre_logit
        pre_logit = self.dense(cog_ref_ctx)
        ...
        if not (config.woDiv):
            ...
            loss = emo_loss + 1.5 * div_loss + ctx_loss + kll_loss + intent_loss
        else:
            loss = emo_loss + ctx_loss + kll_loss + intent_loss
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        src_mask, emo_ref_ctx,ctx_output, _ = self.forward(batch)
        ...
        for i in range(max_dec_step + 1):
            ...
            if config.project:
                out, attn_dist = self.decoder(self.embedding_proj_in(ys_embed),
                                              self.embedding_proj_in(ctx_output),
                                              (src_mask, mask_trg),
                                              )
            else:
                out, attn_dist = self.decoder(ys_embed, emo_ref_ctx, ctx_output, (src_mask, mask_trg))
                # 内容意图
                pre_logit, attn_dist = self.decoder_1(ys_embed, ctx_output, (src_mask, mask_trg))
                pre_logit_int, attn_dist = self.decoder_2(pre_logit, z_prior_state.unsqueeze(1), (None, mask_trg))
                # 最终生成
                pre_logit = torch.cat([pre_logit_int, out], dim=-1)
                cog_contrib = nn.Sigmoid()(pre_logit)
                cog_ref_ctx = cog_contrib * pre_logit
                pre_logit = self.dense(cog_ref_ctx)
                out = pre_logit
                #out, attn_dist = self.decoder_2(out, z_prior_state.unsqueeze(1), (None, mask_trg))
